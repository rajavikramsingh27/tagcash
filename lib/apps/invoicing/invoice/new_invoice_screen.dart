...
class NewInvoicingScreen extends StatefulWidget {
  @override
  _NewInvoicingScreenState createState() => _NewInvoicingScreenState();
}

class _NewInvoicingScreenState extends State<NewInvoicingScreen> {
...
// get current date
  ...
  void getDefault() async {
    ...
    if (response['status'] == 'success') {
      ...
      setState(() {
        default_id = jsonn[0]['id'];
        default_payment_term = jsonn[0]['payment_terms'];
        default_title = jsonn[0]['default_title'];
        default_subheading = jsonn[0]['default_subheading'];
        default_footer = jsonn[0]['default_footer'];
        default_notes = jsonn[0]['notes'];

        _notesController.text = default_notes;
        _footerController.text = default_footer;
  ...
  Future<List<Item>> getItem(String quantity) async {
      ...
      if (type == '1'){
        ...
        Navigator.of(context).push(
            new MaterialPageRoute(builder: (context) => InvoicePreviewScreen(invoice_id:invoice_id, invoice: invoice_no, amount_due: total_amount.toStringAsFixed(2), due_on: paymentDate,
                ...
                subtotal: subTotal.toString(), setTaxData: setTaxData, totalamount: getTotal(), type: 'new'),
                                  ...
                                  children: [
                                    ...
                                    Container(
                                      child: getData[index].taxx.length != 0 && getData[index].taxx.length != null
                                          ...
                                          style: Theme.of(context).textTheme.subtitle2.apply(color: Color(0xff203354))
                                      ),
