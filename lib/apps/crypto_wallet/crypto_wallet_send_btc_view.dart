...
class _CryptoWalletSendBTCView extends State<CryptoWalletSendBTCView> {
  ...
  void createTransaction() async {
    try {
      ...
      //   showError(tXSkeleton.errors.first);
      // } else {
      //   showConfirmView();
      // }
    } catch (err) {
  ...
  confirmTransaction() async {
    ...
    try {
      // tx = await tagbondModel.confirmBTCTransaction(
      //     toAddress,
      //     double.parse(amount),
      //     tXSkeleton.tx.inputs.first.prev_hash,
      //     tXSkeleton.tx.inputs.first.output_index);
      // print("BTC transact hash => ${tx.hash}");
      // setState(() {
    ...
    return Container(
        ...
        child: Column(
          children: <Widget>[
            ...
            // Align(
            //     alignment: Alignment.centerLeft,
            //     child: Text(tXSkeleton.tx.fees.toString(),
            //         style: TextStyle(color: kTextLightColor, fontSize: 15))),
            SizedBox(height: 15),
            Align(
                alignment: Alignment.centerLeft,
                    ...
                    child: new LayoutBuilder(builder:
                      ...
                      return Row(
                        ...
                        children: <Widget>[
                          ...
                          AnimatedContainer(
                            height: 50,
                            width: isLoading ? 50 : constraints.maxWidth / 2.1,
                            duration: Duration(milliseconds: 200),
                            decoration: BoxDecoration(
                              color: kPrimaryColor,
                              borderRadius: BorderRadius.circular(26),
  ...
  getSuccessView() {
    return Container(
        ...
        child: Column(children: <Widget>[
          ...
          Text(getTranslated(context, "btc_transafer_to_address"),
              textAlign: TextAlign.center,
              style: TextStyle(
                  fontSize: 17,
                  fontWeight: FontWeight.w700,
...
Dialog showBTCSendView(BuildContext context, Wallets defaultWallet) {
  return Dialog(
