...
class CreateNewsFeedScreen extends StatefulWidget {
  _CreateNewsFeedScreenState createState() => _CreateNewsFeedScreenState();
}
  ...
  Widget showdiag(BuildContext context, data, String dialogType) {
    return Dialog(
      ...
      child: SingleChildScrollView(
        ...
        child: dialogType == '0' ? dialogContent(context, data) : data,
      ),
    );
  }
...
//      imageFile = selectedLogo.path;
  ...
  void createNewsFeed(name) async {
    setState(() {
      isLoading = true;
    });

    Map<String, String> apiBodyObj = {};
    apiBodyObj['news_feed_text'] = name;

    Map<String, dynamic> response =
        await NetworkHelper.request('NewsFeed/CreateNewsFeed', apiBodyObj);
                      ...
                      onPressed: () async {
                        ...
                        if (videoFile != '') {
                          if (_formKey.currentState.validate()) {
                            createNewsFeed(newsfeedText.text);
                          }
                        } else {
                          if (isImage == false) {
